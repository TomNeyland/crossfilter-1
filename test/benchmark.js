...
function runBenchmark(opts) {
  ...
  var useString = opts.useStringAccessor;
  ...
  var payments = crossfilter(firstBatch),
      ...
      amount = payments.dimension(useString ? 'amount' : function(d) { return d.amount; }),
      amounts = amount.group(Math.floor),
      date = payments.dimension(useString ? 'date' : function(d) { return d.date; }),
      dates = date.group(d3.time.day),
      day = payments.dimension(useString ? 'date.getDay' : function(d) { return d.date.getDay(); }),
      days = day.group(),
      hour = payments.dimension(useString ? 'date.getHours' : function(d) { return d.date.getHours(); }),
      hours = hour.group();
...
console.log('Running benchmark with string accessors:');
runBenchmark({useStringAccessor: true});
// run the benchmarks
